...
INSERT 语句通常**不会立即生成锁结构**，而是依靠：

- **聚簇索引记录的 trx_id 隐藏列**作为隐式锁保护
- **只有发生冲突时，才转换为显式锁**
...
1. 情况1：存在间隙锁时
   ...
   -- 事务B：尝试插入
   INSERT INTO users(id, name) VALUES(15, '张三');  -- 被阻塞
   ```

   **锁机制：**

   - 事务B会申请**插入意向锁(Insert Intention Lock)**

   - 插入意向锁与间隙锁**冲突**，状态设为**等待**
...
2. 情况2：无间隙锁时
   ...
   ```sql
   -- 事务B：直接插入
   INSERT INTO users(id, name) VALUES(15, '张三');  -- 立即执行
   ```
...
-- 事务A：插入重复主键
INSERT INTO users(id, name) VALUES(10, '王五');  -- 第一个事务
事务主键索引锁结果
事务A(首个)对现有记录id=10加 X型记录锁(显式)报错：Duplicate entry
事务B(后续)尝试对现有记录id=10加 S型记录锁阻塞：等待事务A释放X锁
```

2. 唯一二级索引冲突

```sql
-- 现有数据：email='test@example.com'(id=10)

-- 事务A：插入重复唯一索引
INSERT INTO users(id, name, email) VALUES(20, '张三', 'test@example.com');
事务唯一二级索引锁结果
事务A(首个)现有记录的隐式锁转为X型记录锁报错：Duplicate entry
事务B(后续)尝试对现有记录加 S型Next-Key锁阻塞：等待事务A释放锁
```
...
```sql
-- 事务A、B同时插入相同的普通索引值
INSERT INTO users(id, name, age) VALUES(20, '张三', 25);
INSERT INTO users(id, name, age) VALUES(21, '李四', 25);  -- age是普通索引
```

| 索引类型         | 第一个事务                                        | 之后事务                                                     |
| ---------------- | ------------------------------------------------- | ------------------------------------------------------------ |
| **主键**         | 对**已存在记录**加 X型记录锁(显式)<br/>→ 报错     | 阻塞：尝试对**已存在记录**加 S型记录锁                       |
| **唯一二级索引** | 对**已存在记录**的隐式锁转为 X型记录锁<br/>→ 报错 | 阻塞：尝试对**已存在记录**加 S型Next-Key锁（取巧设计，对业务影响不大） |
| **普通二级索引** | 隐式锁保护**新插入记录**<br/>→ 成功               | 隐式锁保护**新插入记录**<br/>→ 成功                          |
...
- **commit 阶段**：把 XID 写入到 binlog，然后将 binlog 持久化到磁盘（sync_binlog = 1 的作用），接着调用引擎的提交事务接口，将 redo log 状态设置为 commit，此时该状态并不需要持久化到磁盘，只需要 write 到文件系统的 page cache 中就够了，因为只要 binlog 写磁盘成功，就算 redo log 的状态还是 prepare 也没有关系，一样会被认为事务已经执行成功
  - **commit阶段，执行 `commit` 语句时，binlog cache 刷写到 binlog日志，并将redo-log日志修改为commit状态**
